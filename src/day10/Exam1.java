...
public class Exam1 {
    public static void main(String[] args) {
        ...
        // 3. 참새객체
        참새 sparrow1 = new 참새(); // 참새생성자 + 조류생성자 + 동물 생성자
        sparrow1.name = "참새";
        sparrow1.show();
        
        // 4. 닭객체
        닭 chicken1 = new 닭();     // 닭 생성자 + 조류 생성자 + 동물 생성자 실행
        chicken1.name = "닭";
        chicken1.show();
        ...
        // 1. 참새가 조류로 타입 변환 될까?
        // 가능한 이유: 1) 상속관계(논리) 2) 참새인스턴스가 생성될때 + 조류 + 동물인스턴스도 생성(물리)
        조류 bird2 = sparrow1;      // 자동타입변환 / 업캐스팅(올라가기)
        // 2. 참새 타입에서 동물 타입으로 될까?
        동물 animal2 = sparrow1;    // 업캐스팅
        // 3. 동물 타입에서 참새 타입으로 될까? 주의!!!!!
        // 주의할점 : 1) 변환타입명명시     2) 변환할 자료가 변환할 타입 포함하는지?
        참새 sparrow2 = (참새)animal2;  // 강제타입변환 / 다운캐스팅(내려가기)
        // 4. 동물 타입에서 참새 타입으로 될까? 주의!!!!!!!!
        // 즉] 자식이 태어날때 부모도 태어나지만 자식은 태어나지 않음. 
        참새 sparrow3 = (참새)animal1;  // 오류발생 동물1 태어날때 참새가 생성되지 않았음.
